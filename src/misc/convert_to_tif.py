...
def convert_to_tif():
	root = Path(__file__).parent.absolute() / '..' / '..' / 'dataset'
	tif_test_dir = root / 'test_tif'
	tif_train_dir = root / 'train_tif'
	tif_val_dir = root / 'val_tif'
	...
	for name in source_dirs.keys():
		...
		for f in tqdm(list(files)):
			...
			save_path = _get_save_path(name, f, root)
			io.imsave(str(save_path), tif_im)
	print(skip_count)

def _get_save_path(name, f, root):
