...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.soft_encoding:
            ...
            Y = torch.tensor(np.moveaxis(Y, 2, 0),
                             dtype=torch.float32) / self.kernel_normalize  # switch to channel first format
        return X, Y
...
def prepare(set_spec, params):
    ...
    train_subset = torch.utils.data.Subset(dataset, indices = range(0, int(len(dataset)/8)))

    train_loader = torch.utils.data.DataLoader(train_subset, batch_size=batch_size, num_workers=num_workers,
                                               shuffle=shuffle, drop_last=False, pin_memory=True)

    return train_loader


def return_loaders(batch_size=64, num_workers=5, shuffle=True, soft_encoding=True):
